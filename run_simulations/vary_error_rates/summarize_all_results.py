...
for error_rate in error_rates:
    for s in seed:
        diamond_fast_file = "diamond_fast_output/diamond_performance_metrics_" + str(error_rate) + "_seed_" + str(s)
...
for error_rate in error_rates:
    for s in seed:
        for ksize in [11, 15]:
            sourmash_file = "sourmash_output/sourmash_performance_metrics_" + str(error_rate) + "_seed_" + str(s) + "_k_" + str(ksize)
...
for error_rate in error_rates:
    for s in seed:
        diamond_fast_file = "diamond_fast_output/diamond_resource_usage_" + str(error_rate) + "_seed_" + str(s)
        if os.path.exists(diamond_fast_file):
...
for error_rate in error_rates:
    for s in seed:
        for ksize in [11, 15]:
            sourmash_file = "sourmash_output/resource_usage_" + str(error_rate) + "_seed_" + str(s) + "_k_" + str(ksize)
            if os.path.exists(sourmash_file):
