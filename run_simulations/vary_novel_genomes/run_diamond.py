...
def main():
    ...
    for num_novel_genome in [0, 2, 4, 8, 16, 32]:
        for i in range(50):
            ...
            gene_output = "diamond_fast_output/diamond_gene_results_" + str(num_novel_genome) + "_seed_" + str(seed)
            ko_output = "diamond_fast_output/diamond_ko_results_" + str(num_novel_genome) + "_seed_" + str(seed)
            resource_output = "diamond_fast_output/diamond_resource_usage_" + str(num_novel_genome) + "_seed_" + str(seed)
            
            # check if all the output files exist. if yes, skip this run
            if os.path.exists(diamond_output) and os.path.exists(gene_output) and os.path.exists(ko_output) and os.path.exists(resource_output):
                continue
