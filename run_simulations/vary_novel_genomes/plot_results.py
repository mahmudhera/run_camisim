...
merged_resources_data = pd.merge(mean_resources_data, std_resources_data, on=['num_novel_genomes', 'tool'], suffixes=('_mean', '_std'))
...
for i, metric in enumerate(resource_metrics, 1):
    ...
    lineplot = sns.lineplot(
        x='num_novel_genomes', y=f'{metric}_mean', hue='tool', style='tool',
    ...
    for j, tool in enumerate(merged_resources_data['tool'].unique()):
        ...
        plt.errorbar(subset['num_novel_genomes'], subset[f'{metric}_mean'], yerr=subset[f'{metric}_std'], 
                     fmt='none', capsize=5, label='_nolegend_', color=lines[j].get_color())

    plt.xlabel('Number of novel genomes')
    ...
    plt.xticks(subset['num_novel_genomes'].unique(), labels=subset['num_novel_genomes'].unique())
